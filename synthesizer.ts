...
export class Synthesizer {
  ...
  async synthesize(query: string, findings: string[], citations: Citation[]): Promise<Omit<ResearchReport, "metadata">> {
    if (!findings || findings.length === 0) {
      return {
        query,
        summary: "Insufficient data to generate a comprehensive report.",
        content: "No findings were available to synthesize a report. Please try a different query or check your search sources.",
        citations: [],
      };
    }
    ...
    try {
      const response = await withRetry(() => this.ai.models.generateContent({
        ...
        Citations: ${JSON.stringify(citations.map((c, i) => ({ index: i + 1, ...c })))}
      ...
      if (!report.query || !report.summary || !report.content) {
      ...
      return {
        query,
        summary: "An error occurred during report synthesis.",
        content: `# Error\n\nFailed to synthesize the research report. Error: ${err instanceof Error ? err.message : "Unknown error"}`,
        citations: citations || [],
      };
    }
  }
