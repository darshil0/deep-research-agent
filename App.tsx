...
import { Search, Loader2, CheckCircle2, XCircle, Clock, Hash, FileText, Settings, ExternalLink, ChevronRight, Info, AlertCircle } from "lucide-react";
...
export default function App() {
  ...
  const connectWebSocket = (id: string) => {
    ...
    const ws = new WebSocket(`${protocol}//${window.location.host}?taskId=${id}`);
    wsRef.current = ws;
    setWsError(null);
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (update.status === "completed") {
          ...
          setIsResearching(false);
          toast.error(update.error || "Research failed");
          ws.close();
        }
      } catch (err) {
    ...
    ws.onclose = () => {
      ...
      if (isResearching && state?.status !== "completed" && state?.status !== "failed") {
      ...
      <header id="main-header" className="border-b border-white/10 bg-black/50 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <div className="text-[10px] font-mono text-white/40 uppercase tracking-widest hidden sm:block">v1.5.0</div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        {wsError && (
          <motion.div 
            ...
            className="mb-6 p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-xl flex items-center gap-3 text-yellow-400"
          >
              ...
              {showSettings && (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    <div className="space-y-2">
                      <label htmlFor="max-iterations" className="text-xs text-white/40 uppercase tracking-wider font-semibold">Max Iterations</label>
                      <input 
                        id="max-iterations"
                        type="number" 
                        min="1"
                    ...
                    <div className="space-y-2">
                      <label htmlFor="time-limit" className="text-xs text-white/40 uppercase tracking-wider font-semibold">Time Limit (s)</label>
                      <input 
                        id="time-limit"
                        type="number"
                        min="60"
                        ...
                        onChange={(e) => setConfig({ ...config, maxTimeSeconds: parseInt(e.target.value) || 300 })}
                        className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500"
                      />
                    </div>
                  </div>
              ...
              {!state?.report ? (
                ...
                >
                  ...
                  <div className="p-6 sm:p-8 space-y-10 sm:space-y-12">
                    ...
                    {state.report.citations && state.report.citations.length > 0 && (
